...
export function setup() {
    ...
    let cartIds = []; // Array para armazenar todos os IDs de carrinhos
    ...
    for (let i = 0; i < 20; i++) {
        ...
        cartIds.push(cartId); // Armazena o ID do carrinho

        tokens.push(authToken);
    }
    
    // Retorne um objeto contendo tokens, productIds e cartIds
    return { tokens, productIds, cartIds };
}

export default function () {
    try {
        // Exemplo: Get a cart by ID
        const cartId = __ENV.CART_ID || ''; // Assumindo que o ID do carrinho é passado como variável de ambiente
        if (cartId) {
            const getCartByIdRes = baseRest.get(`${ENDPOINTS.CARTS_ENDPOINT}/${cartId}`);
            baseChecks.checkStatusCode(getCartByIdRes, 200);
            console.log(`Detalhes do carrinho com ID ${cartId}: ${getCartByIdRes.body}`);
        }

        // Geralmente, você pode querer fazer outras operações aqui
